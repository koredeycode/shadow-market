...
const createMarketSchema = z.object({
  ...
  oracleAddress: z.string().startsWith('mn_shield-addr_', 'Invalid oracle address format'),
  initialLiquidity: z.string().refine(val => !isNaN(parseFloat(val)) && parseFloat(val) >= 10, {
    message: 'Minimum initial liquidity is 10 tDUST',
  }),
});
...
export function CreateMarket() {
  ...
  const {
    ...
    defaultValues: {
      category: 'Politics',
      oracleAddress: 'mn_shield-addr_undeployed1pfytaphcp072fckzjqx5y2cd6yzy5jzz802gjev4w2vw97mjf0deny9jp0j527nrh2y326dr5x989fq4tdgldj5nula84d3s2y7pvmscsec3u',
    ...
    try {
      ...
      if (txHash) {
        ...
        try {
          ...
          toast.success('Market synchronized successfully');
          navigate(`/markets/${market.slug || market.id}`);
        } catch (syncError) {
          console.error('DEBUG: Backend synchronization failed:', syncError);
          toast.error('Market live on-chain, but dashboard sync delayed.');
          navigate('/markets');
        }
      } else {
        console.warn('DEBUG: createMarket returned null txHash. Market may not have been created.');
      }
    } catch (error: any) {
                      ...
                      <div className="pt-4 border-t border-white/5 flex items-center gap-4">
                        ...
                        <div className="flex-1 flex gap-2">
                          <input
                            ...
                            className="flex-1 bg-slate-950 border border-white/10 rounded-sm px-3 py-1.5 text-xs text-white font-mono focus:outline-none focus:border-electric-blue/50"
                          />
                        </div>
                      </div>
                    </div>
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-mono text-slate-500 uppercase tracking-widest flex items-center justify-between">
                  ...
                  {errors.initialLiquidity && (
                    <span className="text-red-400 flex items-center gap-1">
                      <AlertCircle className="w-3 h-3" /> Min 10
                    </span>
                  )}
                </label>
                <input
                  ...
                  type="number"
                  placeholder="100.00"
                  className={`w-full bg-slate-950 border rounded-sm px-4 py-3 text-white focus:outline-none transition-all font-mono text-xl ${
                    errors.initialLiquidity ? 'border-red-500/50' : 'border-white/10 focus:border-electric-blue/50'
                  }`}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className={`w-full bg-slate-950 border rounded-sm px-4 py-3 text-white focus:outline-none transition-all font-mono text-[10px] ${
                    errors.oracleAddress ? 'border-red-500/50' : 'border-white/10 focus:border-electric-blue/50'
                  }`}
                />
              </div>
            </div>
